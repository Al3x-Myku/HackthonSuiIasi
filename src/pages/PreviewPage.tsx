...
const PreviewPage: React.FC = () => {
  ...
  // Redirect safely if user landed here without state (refresh / deep link / mobile quirks)
  useEffect(() => {
    if (!imageSrc) {
  ...
  // If no imageSrc, show a tiny fallback instead of blank/null
  if (!imageSrc) {
    return (
  ...
  const API_URL = useMemo(() => {
    // Recommended: set VITE_API_URL to something reachable from phone
    // Fallback: use same origin if you configure a dev proxy ("/api")
  ...
  const runAiDetection = async (imageBlob: Blob) => {
    ...
    try {
      ...
      // If VITE_API_URL is not set, try same-origin proxy route
      // You can set VITE_API_URL="http://PC_IP:5000" in .env for mobile
  ...
  const handleMint = async () => {
    ...
    try {
      ...
      if (data.newlyCreated?.blobObject?.blobId) blobId = data.newlyCreated.blobObject.blobId;
      else if (data.alreadyCertified?.blobId) blobId = data.alreadyCertified.blobId;
              ...
              {isDetecting ? (
                ...
                <div className="text-xs text-yellow-200">
                  ...
                  <div className="mt-2 text-[11px] text-yellow-200/80">
                    Tip: pe telefon, nu folosi localhost. Setează VITE_API_URL la IP-ul PC-ului.
                  </div>
                </div>
              ) : (
                <div>
